            ...
            for ( ; i < lines.Length; i++)
            {
                ...
                foreach (string item in columnOfGivenLine)
                {
                    try
                    {
                        ...
                        {
                            if (l < 4 && (String.IsNullOrWhiteSpace(item) || String.IsNullOrEmpty(item)))
                            {
                                ///First 4 values cannot be null empty etc
                                Console.WriteLine("Missing value for \'{0}\' is neccessary!",propertyInfo.Name);
                                create = false;
                                break;
                            }
                        ...
                        {
                            ///fill 0 if input string is null or whitespace
                    ...
                    {
                        Console.WriteLine("Unable to convert rest of the file : " + l.ToString() + " " + ex.Message);
                        break;
                    }
